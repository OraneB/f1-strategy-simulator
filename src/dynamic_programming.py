...
def best_strategy_dynamic_programming(race_length, tire_compounds, min_stint_length = 5, pit_stop_loss = 22.0):
    """
    ...
    def dynamic_programming(current_lap, compound, tire_age, compounds_used: frozenset):
        ...
        best_option = (remaining_time + tire_compounds[compound].laptime_on_lap(tire_age), remaining_strategy)
        if tire_age >= min_stint_length:
            for new_compound in tire_compounds:
                ...
                pit_stop_time = remaining_time + pit_stop_loss + tire_compounds[new_compound].laptime_on_lap(0)
